...
with open('src/pages/CustomerMenu.tsx', 'r', encoding='utf-8') as f:
    content = f.read()

# 1. Imports
content = content.replace("ShoppingCart\n} from 'lucide-react';", "ShoppingCart,\n    Search\n} from 'lucide-react';")

# 2. State
content = content.replace("const [loadingMenu, setLoadingMenu] = useState(true);", "const [loadingMenu, setLoadingMenu] = useState(true);\n    const [searchQuery, setSearchQuery] = useState('');")
...
"""
content = content.replace("    // --- Save cart as image (walk-in) ---", scroll_logic + "    // --- Save cart as image (walk-in) ---")

# 4. Header Replacement
header_pattern = re.compile(r"\{\/\* ── Header ── \*\/}.*?\{\/\* ── Menu List ── \*\/}", re.DOTALL)
new_header = """{/* ── Header ── */}
...
# 5. Extract original item rendering block
item_card_pattern = re.compile(r"(<div key=\{item\.id\}.*?</div>\s*</div>\s*</div>)", re.DOTALL)
match = item_card_pattern.search(content)
if not match:
    print("Could not find item card logic!")
    exit(1)
card_jsx = match.group(1)

# 6. Helper logic
helpers = """    const isItemInCat = (item: any, catId: string) => {
...
# 7. Main List logic replace
main_list_pattern = re.compile(r"\{/\* ── Menu List ── \*/}.*?<main.*?</main>", re.DOTALL)
new_main = """{/* ── Menu List ── */}
...
# Using replace to inject card JSX without f-strings issues
new_main = new_main.replace("REPLACE_CARD_JSX", card_jsx)
content = main_list_pattern.sub(new_main, content)


# 8. Fix Call Waiter Alert
content = content.replace("alert('Could not notify staff. Please try again or wave to a waiter.');", "alert(`Could not notify staff: ${error.message || JSON.stringify(error)}`);")
content = content.replace("catch (error) {", "catch (error: any) {")

with open('src/pages/CustomerMenu.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
