...
const newHeader = `            {/* ── Header ── */}
            <header className="sticky top-0 z-40 bg-background/95 backdrop-blur-md border-b border-border shadow-sm">
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <div className="flex flex-col">
                                ...
                                <span className="text-[10px] text-muted-foreground font-black tracking-widest uppercase bg-secondary/80 self-start px-2 py-0.5 rounded border border-border">
                ...
                {!searchQuery && (
                    <div className="flex overflow-x-auto hide-scrollbar pb-2 px-4 gap-2 bg-background">
                        {loadingMenu ? (
                            ...
                            topLevelCats.map(cat => (
                                <button
                                    ...
                                    className={\`whitespace-nowrap px-3 py-1.5 rounded-full text-xs font-bold transition-all border \${activeCategory === cat.id
...
// 5. Helper Methods (isItemInCat, searchResults) before renderStars
const helperMethods = `
...
`;
content = content.replace('    const renderStars =', helperMethods + '    const renderStars =');

// 6. Main List Replacement
const mainStart = content.indexOf('            {/* ── Menu List ── */}');
const cartStart = content.indexOf('            {/* ── Cart Sliding Overlay ── */}');

// Extract the card JSX from original content
let origMain = content.substring(mainStart, cartStart);
let cardStart = origMain.indexOf('<div key={item.id}');
let cardEnd = origMain.lastIndexOf('</div>', origMain.indexOf('))', cardStart)) + 6;
let cardJSX = origMain.substring(cardStart, cardEnd);

const newMainBody = `            {/* ── Menu List ── */}
