...
const CustomerMenu = () => {
    ...
    // activeSubCategory removed — subcategory nav replaced by scrollspy sections
        ...
        const handleScroll = () => {
            if (!isScrolling) {
                window.requestAnimationFrame(() => {
                    ...
                    let currentCat = topLevelCats[0]?.id;
                    ...
                    if (currentCat && currentCat !== lastCatId) {
                        lastCatId = currentCat;
                        const tabEl = document.getElementById(`tab-${currentCat}`);
                        if (tabEl && tabEl.parentElement) {
                            const container = tabEl.parentElement;
                            const scrollLeftTarget = tabEl.offsetLeft - (container.clientWidth / 2) + (tabEl.clientWidth / 2);
    ...
    // Subcategories of the currently active top-level category (kept for potential future use)
    ...
    return (
        <div className="min-h-screen bg-background text-foreground pb-36 font-sans selection:bg-primary/20 relative">
            ...
            <header className="sticky top-0 z-40 bg-background/95 backdrop-blur-md border-b border-border shadow-sm">
                <div className="px-4 py-2 flex flex-col gap-2">
                        ...
                        <div className="flex items-center gap-2">
                            {activeOrders.length > 0 && (
                                <button
                                    ...
                                    className="p-1.5 bg-primary/10 text-primary rounded-lg relative hover:bg-primary/20 transition-all border border-primary/20"
                                >
                                    <Bell size={18} className="animate-pulse" />
                                    <span className="absolute -top-1 -right-1 w-4 h-4 bg-primary text-white text-[9px] flex items-center justify-center rounded-full font-black">
                                        {activeOrders.length}
                                    </span>
                                </button>
                            )}
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full bg-secondary/60 border border-border/60 rounded-lg pl-9 pr-8 py-1.5 text-sm outline-none focus:border-primary/50 transition-colors"
                        />
                        {searchQuery && (
                ...
                {/* Main Category Tabs */}
                <div className="flex overflow-x-auto hide-scrollbar pb-2 px-4 gap-2 bg-background border-t border-border/30 pt-2">
                    {loadingMenu ? (
            ...
            <main className="max-w-3xl mx-auto p-4 space-y-8">
                ...
                {loadingMenu ? (
                    ...
                    /* Category Sections — all rendered so scrollspy works */
                    topLevelCats.map(cat => {
                        const itemsInCat = menuItems.filter(i => i.is_active !== false && isItemInCat(i, cat.id));
                        if (itemsInCat.length === 0) return null;

                        const subCats = categories.filter(c => c.parent_id === cat.id);
                        const directItems = itemsInCat.filter(i => i.category_id === cat.id);
                            ...
                            <section id={`cat-${cat.id}`} key={cat.id} className="scroll-mt-40">
                                    ...
                                    <div className="flex flex-wrap justify-center gap-2 mb-6 px-1">
                                        {subCats.map(sub => (
                                            <button 
                                                ...
                                                className="px-4 py-2 bg-background border border-border shadow-sm rounded-full text-xs font-bold text-foreground hover:bg-secondary transition-colors"
                                            >
                                                {sub.name}
                                            </button>
                                        ))}
                                ...
                                <div className="space-y-8">
                                    {directItems.length > 0 && (
                                        <div className="space-y-4">
                                            {directItems.map(item => renderMenuItem(item))}
                                        </div>
                                    )}
                                    
                                    {subCats.map(sub => {
                                        const subItems = itemsInCat.filter(i => i.category_id === sub.id || i.extra_category_ids?.includes(sub.id));
